...
- **공공도서관 대상 B2B SaaS 기업** 백엔드 개발자 | 2024.01 ~ 재직 중 (2년 7개월)
...
- **30개 도서관·키오스크 65대에서 일평균 약 2,700건**을 처리하는 서비스와, **월 60만~180만 건**의 대출 이력을 다루는 배치 파이프라인을 운영했습니다.
- 현장 설치 과정을 스크립트로 자동화해 **기관당 약 1시간 걸리던 설치를 5분으로 단축**하고, 비개발자도 설치할 수 있는 체계를 4개 제품에 적용했습니다.
- **AI 코딩 에이전트(Claude Code)를 실제 업무 브랜치에 도입**해 기능 개발·버그 수정·문서화에 활용하고 있습니다.
...
| 프로젝트 | 기간 | 요약 |
|---|---|---|
| [1. 마인드픽 — AI 그림심리검사 기반 도서추천 서비스](projects/01-mindpick.md) | 2025.07 ~ 2026.02 | 백엔드·게이트웨이·인프라 전 영역 단독 개발. OpenAI Responses API + SSE 스트리밍 + RAG, GS 품질인증 획득 |
| [2. 현장 설치·배포 자동화](projects/02-install-automation.md) | 2024 ~ | 4개 제품에 적용, 기관당 설치 시간 1시간 → 5분 |
| [3. 북메이트 — 도서관 무인 대출·추천 키오스크 플랫폼](projects/03-bookmate.md) | 2024.01 ~ 재직 중 | 30개 도서관·키오스크 65대, 일평균 약 2,700건 처리 |
| [4. AI 미들웨어(북큐레이션) — 도서관 정보시스템 연계 서비스](projects/04-ai-middleware.md) | 2025.06 ~ 2026.07 | RFID 인증, 개인화 추천 연계, 설치·배포 체계 인수 |
| [5. 도서 추천 시스템 — 대출 이력 기반 추천 데이터 파이프라인](projects/05-recommend-pipeline.md) | 2024.01 ~ 2026.06 | Spring Batch 기반 ETL, 월 60만~180만 건 처리 |
...
- **테스트 코드** — 북메이트에서 문학상 수상작 도메인과 외부 연계 서비스의 테스트 코드를 작성하며 팀의 테스트 관행에 참여했습니다. 다만 **단독 개발한 마인드픽은 자동화 테스트 커버리지가 낮고**, GS인증 기능시험과 API 명세 기반 회귀 검증으로 품질을 확보했습니다. **테스트 자동화 확대를 다음 과제로 두고 있습니다.**
