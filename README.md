...
공공도서관 대상 B2B 설치형 소프트웨어 4종의 서버를 설계·운영하고, AI 심리검사 서비스를 백엔드·게이트웨이·인프라까지 단독으로 만들어 GS 품질인증을 받은 백엔드 엔지니어입니다.
...
- 공공도서관 대상 B2B 설치형 소프트웨어 **4종**의 서버를 설계·개발·운영. Java/Spring 기반 **MSA**, **Spring Batch 데이터 파이프라인**, **LLM 연동 서비스**를 담당했습니다.
...
- **북메이트는 30개 도서관·키오스크 65대에서 일평균 약 2,700건**의 키오스크 이용·추천 조회를 처리하고, **도서 추천 시스템은 29개 기관**에 적용되어 **월 60만~180만 건**의 대출 이력을 다루는 배치 파이프라인을 운영하고 있습니다.
...
| [3. 북메이트 — 도서관 무인 추천·정보 키오스크 플랫폼](projects/03-bookmate.md) | 2024.01 ~ | 30개 도서관·키오스크 65대, 일평균 약 2,700건의 키오스크 이용·추천 조회 처리 |
...
- **테스트 코드** — 북메이트에서 문학상 수상작 도메인과 SNS 연동 기능의 **API·서비스 계층 테스트 코드**를 작성하며 팀의 테스트 관행에 참여했습니다. 마인드픽은 **GS인증 기능시험 전 과정과 API 명세 기반 회귀 검증**으로 품질을 확보했습니다.
